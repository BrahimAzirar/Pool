...
export default function piscine() {
  ...
  /*
  `references_pool`, `offer`, `add_person`
  */
  ...
  const deletePiscine = (id) => {
    ...
    axios.post(`/api/pools/${id}`,params)
    .then((response) => {
      setOffer('')
      setPerson('')
      fetchPiscine()
                  ...
                  <form onSubmit={submitForm} method='Post'>
                    <div class="top-content">
                        ...
                        <p>Choisissez L'offer:</p>
                          <input type="number" className="salle" placeholder='ajoute offer....'
                          name="offer"
                          value={offer}
                        ...
                        <p>Autres piscine:</p>
                          <input type="number" className="salle" placeholder='ajoute autre....'
                          name="person"
                          value={person}
  ...
  <input onClick={generatePDF} type="button" value="PDF" />
    ...
    {piscine.map(item=>(
      <tr key={item.id}>
        ...
        <td>{item.total}</td>
        <td>
          <button onClick={()=> editPiscine(item.id)}>update</button>
          <button onClick={()=> deletePiscine(item.id)}>delete</button>
        </td>
      </tr>
    ))}
    </tbody>
