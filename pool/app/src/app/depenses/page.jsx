...
export default function depenses() {
  ...
  const deleteDepenses = (id) => {
    ...
    axios.post(`/api/depenses/${id}`,params)
    .then((response) => {
      setPrice('')
      setName('')
      fetchDepense()
                ...
                <form method='Post' onSubmit={submitForm}>
                  <div className="top-content">
                      ...
                      <p>Le type de depenses:</p>
                        <input type="text" className="salle" placeholder='le type de depenses....'
                        name="name"
                        value={name}
                        onChange={nameChange}
                        />
...
<div className="bottom-content">
  ...
  {depenses.map((item, i)=>(
      ...
      <td>{item.name}</td>
      <td>
        <button onClick={()=> editDepenses(item.id)}>update</button>
        <button onClick={()=> deleteDepenses(item.id)}>delete</button>
      </td>
  </tr>
    ))}
